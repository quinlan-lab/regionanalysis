...
#python residualgist/resid-plot.py $DATA/exac-resid.txt > residualgist/fullresiduals.txt
#cat <(head -1 residualgist/fullresiduals.txt) <(sed '1d' residualgist/fullresiduals.txt | bash dups.sh) > residualgist/residuals.txt
FILE=$1;FOLDER=$2
#sed '1d' residualgist/residuals.txt | awk '$12 >= 99' > regions/topresid.txt
#python middle.py > regions/midresid.txt
#awk 'NR==FNR{l+=$3-$2;next} {s+=$3-$2; if (s>=l) exit; else print}' regions/topresid.txt <(sed '1d' residualgist/residuals.txt | awk '$12 <= 10' | sort -k9,9n) > regions/bottomresid.txt
