...
for line in f:
    ...
    frac1=transfrac(fields[1]); frac2=transfrac(fields[2]);
    #frac3=transfrac(fields[3]); frac4=transfrac(fields[4]);
 #   frac5=transfrac(fields[5]); frac6=transfrac(fields[6])
    d[fields[0]]=frac1,frac2#,frac5,frac6#frac3,frac4,frac5,frac6
...
rects2 = ax.barh(ind+width,[i[1] for i in d.values()], width-.05, color = 'dodgerblue', lw=2, alpha=0.7) # hatch='|') #was i[2]
#rects3 = ax.barh(ind,[i[1] for i in d.values()], width-.05, color = 'k', lw=2, alpha=0.8)
...
ax.set_xlabel('Fraction of Essential Genes Overlapped')
ax.set_xlim(0,1)
...
#ax.legend((rects2[0], rects1[0], rects3[0]), ('expected', 'top 1% CCRs', 'pLI > 0.9'),loc='lower right')
ax.legend((rects1[0], rects2[0]), ('top 1% CCRs', 'pLI > 0.998'),loc='lower right')
