...
for line in f:
    ...
    #frac7=transfrac(fields[7]); frac8=transfrac(fields[8])
    d[fields[0]]=frac1,frac2,frac3,frac4,frac5,frac6,#frac7,frac8
...
#rects5 = ax.barh(ind+width*2,[i[4] for i in d.values()], width-.05, color = 'w', lw=2, hatch='//', alpha=0.7)
#rects6 = ax.barh(ind+width*2,[i[5] for i in d.values()], width-.05, color = 'k', lw=2, alpha=0.8)
#rects7 = ax.barh(ind+width*2,[i[6] for i in d.values()], width-.05, color = 'darkorange', lw=2, alpha=0.7) # hatch='x')
#rects8 = ax.barh(ind+width*2,[i[7] for i in d.values()], width-.05, color = 'k', lw=2, alpha=0.8) # hatch='x')
...
ax.legend((rects3[0], rects1[0], rects2[0]), ('top 5% CCRs', 'top 1% CCRs', 'random'),loc='upper right')
#ax.legend((rects4[0],rects7[0], rects2[0], rects1[0]), ('random', 'pLI', 'top 5% CCRs', 'top 1% CCRs'), loc='lower right')
#ax.legend((rects3[0], rects2[0], rects1[0], rects4[0]), ('top 10% CCRs', 'top 5% CCRs', 'top 1% CCRs', 'random'))
...
autolabel(rects1,rects2); autolabel(rects3,rects4);
#autolabel(rects1,rects4); autolabel(rects2,rects5); autolabel(rects7,rects8)#autolabel(rects3,rects6)
