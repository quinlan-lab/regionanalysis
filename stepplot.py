...
def step_plot(patho, benign, ax, ax2, **kwargs):
    p, p_edges = np.histogram(patho, bins=kwargs.pop('bins', 50), range=[patho.min(), patho.max()])
    ...
    b, b_edges = np.histogram(benign, bins=kwargs.pop('bins', 50), range=[benign.min(), benign.max()])
