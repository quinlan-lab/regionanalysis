...
python $HOME/software/pathoscore/pathoscore.py evaluate --functional --prefix $HOME/public_html/pathoscore/clinvar -s CCR -s CADD -s GERP -s MPC -s REVEL -s MTR --suffix pdf pathogenic.combine.vcf.gz benign.combine.vcf.gz #-s gnomad5x1_ccr -s gnomad5x9_ccr -s gnomad50x1_ccr -s gnomad50x9_ccr -s gnomad30x5_ccr
...
bedtools intersect -a <(zcat exacresiduals/gnomad10x.5syn-ccrs.bed.gz | awk '$14>=95') -b pathogenic.combine.vcf.gz -wao > all-ccr-95-patho.txt
bedtools intersect -a <(zcat exacresiduals/gnomad10x.5syn-ccrs.bed.gz | awk '$14>=99') -b pathogenic.combine.vcf.gz -wao > all-ccr-99-patho.txt
...
# run evaluate to generate plot data
python $HOME/software/pathoscore/pathoscore.py evaluate --functional --prefix $HOME/public_html/pathoscore/samocha -s CCR -s CADD -s GERP -s MPC -s REVEL -s MTR --suffix pdf neurodev.combine.vcf.gz control.combine.vcf.gz #-s gnomad5x1_ccr -s gnomad5x9_ccr -s gnomad50x1_ccr -s gnomad50x9_ccr -s gnomad30x5_ccr
