...
if [ ! -s $DATA/CADD.vcf.gz ]; then
    ...
    cat <(grep "^#" $DATA/CADD.exome.vcf) <(cat <(grep -v "^#" $DATA/CADD.exome.vcf) <(grep -v "^#" $DATA/CADDindels.exome.vcf) | sort -k1,1 -k2,2n) | bgzip -c > $DATA/CADD.vcf.gz; tabix -f $DATA/CADD.vcf.gz # exome based only
fi
...
#######TODO TAKE VARIANTS HERE, MULTI DATABASE INTERSECT WITH ALL METRICS $DATA/clinvar-patho-exac.vcf $DATA/clinvar-patho-gnomad.vcf $DATA/clinvar-benign-gnomad.vcf

python caddintersect.py -c $DATA/CADD.vcf.gz -p $DATA/clinvar-patho-exac.vcf $DATA/caddpathos -i 2>/dev/null
python caddintersect.py -c $DATA/CADD.vcf.gz -p $DATA/clinvar-patho-gnomad.vcf $DATA/cadd2pathos -i 2>/dev/null
python caddintersect.py -c $DATA/CADD.vcf.gz -p $DATA/clinvar-benign-gnomad.vcf $DATA/cadd2benigns -i 2>/dev/null
python caddintersect.py -c $DATA/MPC.vcf.gz -p $DATA/clinvar-patho-exac.vcf $DATA/MPCpathos -i 2>/dev/null
python caddintersect.py -c $DATA/MPC.vcf.gz -p $DATA/clinvar-patho-gnomad.vcf $DATA/MPC2pathos -i 2>/dev/null
python caddintersect.py -c $DATA/MPC.vcf.gz -p $DATA/clinvar-benign-gnomad.vcf $DATA/MPC2benigns -i 2>/dev/null

#pathogenic match for ExAC v1 filter
bedtools intersect -a $DATA/clinvar-patho-exac.vcf -b exac-ccrs.bed.gz -header -wa -sorted | bedtools intersect -a stdin -b pli.bed -header -wa -sorted | bedtools intersect -a stdin -b rvis.bed -header -wa -sorted | bedtools intersect -a stdin -b $DATA/caddpathos -header -wa -wb -sorted > tmp/matchtmp
cat <(grep "^#" tmp/matchtmp) <(grep -v "^#" tmp/matchtmp | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t') | bedtools intersect -a stdin -b $DATA/MPCpathos -header -wa -wb -sorted > tmp/matchtmp2
cat <(grep "^#" tmp/matchtmp2) <(grep -v "^#" tmp/matchtmp2 | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t' | uniq) > $DATA/clinvar-patho-exac-match.vcf
#pathogenic match for gnomAD filter
bedtools intersect -a $DATA/clinvar-patho-gnomad.vcf -b gnomad-ccrs.bed.gz -header -wa -sorted | bedtools intersect -a stdin -b pli.bed -header -wa -sorted | bedtools intersect -a stdin -b rvis.bed -header -wa -sorted | bedtools intersect -a stdin -b $DATA/cadd2pathos -header -wa -wb -sorted > tmp/matchtmp
cat <(grep "^#" tmp/matchtmp) <(grep -v "^#" tmp/matchtmp | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t') | bedtools intersect -a stdin -b $DATA/MPC2pathos -header -wa -wb -sorted > tmp/matchtmp2
cat <(grep "^#" tmp/matchtmp2) <(grep -v "^#" tmp/matchtmp2 | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t' | uniq) > $DATA/clinvar-patho-gnomad-match.vcf
#benign match for gnomAD filter
bedtools intersect -a $DATA/clinvar-benign-gnomad.vcf -b gnomad-ccrs.bed.gz -header -wa -sorted | bedtools intersect -a stdin -b pli.bed -header -wa -sorted | bedtools intersect -a stdin -b rvis.bed -header -wa -sorted | bedtools intersect -a stdin -b $DATA/cadd2benigns -header -wa -wb -sorted > tmp/matchtmp
cat <(grep "^#" tmp/matchtmp) <(grep -v "^#" tmp/matchtmp | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t') | bedtools intersect -a stdin -b $DATA/MPC2benigns -header -wa -wb -sorted > tmp/matchtmp2
cat <(grep "^#" tmp/matchtmp2) <(grep -v "^#" tmp/matchtmp2 | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t' | uniq) > $DATA/clinvar-benign-gnomad-match.vcf
...
#exac
###TODO: replace clinvar-patho-exac.vcf with multiintersect, rest of variants can stay the same
cat <(grep '^#' $DATA/clinvar-patho-exac-match.vcf) <(grep -v '^#' $DATA/clinvar-patho-exac-match.vcf | sort -k1,1 -k2,2n) | bgzip -c > $DATA/clinvar-patho-exac.vcf.gz; tabix $DATA/clinvar-patho-exac.vcf.gz
python scorevars.py -x $DATA/clinvar-patho-exac.vcf.gz -c exac-ccrs.bed.gz -a > tmp/ccrpatho
#TODO: replace calc with multiintersect vcf file
EP=$(zgrep -v "^#" $DATA/clinvar-patho-exac.vcf.gz | wc -l)

#gnomAD
###TODO: replace clinvar-patho-gnomad.vcf and benign-gnomad with multiintersect, rest of variants can stay the same
cat <(grep '^#' $DATA/clinvar-benign-gnomad-match.vcf) <(grep -v '^#' $DATA/clinvar-benign-gnomad-match.vcf | sort -k1,1 -k2,2n) | bgzip -c > $DATA/clinvar-benign-gnomad.vcf.gz; tabix $DATA/clinvar-benign-gnomad.vcf.gz
cat <(grep '^#' $DATA/clinvar-patho-gnomad-match.vcf) <(grep -v '^#' $DATA/clinvar-patho-gnomad-match.vcf | sort -k1,1 -k2,2n) | bgzip -c > $DATA/clinvar-patho-gnomad.vcf.gz; tabix $DATA/clinvar-patho-gnomad.vcf.gz
GP=$(zgrep -v "^#" $DATA/clinvar-patho-gnomad.vcf.gz | wc -l)
GB=$(zgrep -v "^#" $DATA/clinvar-benign-gnomad.vcf.gz | wc -l)
...
while getopts ":t:g:c" opt; do
    case $opt in
        ...
        g)
            ...
            python caddintersect.py -c $DATA/CADD.vcf.gz -p $DATA/gnomad-benign-exac-filtered.vcf $DATA/caddbenigns -i 2>/dev/null
            python caddintersect.py -c $DATA/MPC.vcf.gz -p $DATA/gnomad-benign-exac-filtered.vcf $DATA/MPCbenigns -i 2>/dev/null
            #gnomad benign match for ExAC v1 filter
            bedtools intersect -a $DATA/gnomad-benign-exac.vcf -b exac-ccrs.bed.gz -header -wa -sorted | bedtools intersect -a stdin -b pli.bed -header -wa -sorted | bedtools intersect -a stdin -b rvis.bed -header -wa -sorted | bedtools intersect -a stdin -b $DATA/caddbenigns -header -wa -wb -sorted > tmp/matchtmp
            cat <(grep "^#" tmp/matchtmp) <(grep -v "^#" tmp/matchtmp | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t') | bedtools intersect -a stdin -b $DATA/MPCbenigns -header -wa -wb -sorted > tmp/matchtmp2
            cat <(grep "^#" tmp/matchtmp2) <(grep -v "^#" tmp/matchtmp2 | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t' | uniq) > $DATA/gnomad-benign-exac-match.vcf
            #######TODO TAKE VARIANTS HERE, MULTI DATABASE INTERSECT WITH ALL METRICS $DATA/gnomad-benign-exac.vcf

            #exac
            ### TODO: multiintersected variants go in place of gnomad-benign-exac-filtered, rest of the code can remain the same.
            cat <(grep '^#' $DATA/gnomad-benign-exac-match.vcf) <(grep -v '^#' $DATA/gnomad-benign-exac-match.vcf | shuf -n $EP | sort -k1,1 -k2,2n) | bgzip -c > $DATA/gnomad-benign-exac.vcf.gz; tabix $DATA/gnomad-benign-exac.vcf.gz # shuf adds subsampling
        ...
        c)
            ...
            #######TODO TAKE VARIANTS HERE, MULTI DATABASE INTERSECT WITH ALL METRICS $DATA/clinvar-benign-exac.vcf, TODO change wc -l statements to all zgreps so we get filtered version.

            #exac
            ###TODO: replace clinvar-benign-exac.vcf with multiintersect, rest of variants can stay the same
            python secondfilter.py -x $DATA/clinvar-benign-exac.vcf -d genescreens/ad_genecards_clean.txt
            python caddintersect.py -c $DATA/CADD.vcf.gz -p $DATA/clinvar-benign-exac-filtered.vcf $DATA/caddbenigns -i 2>/dev/null
            python caddintersect.py -c $DATA/MPC.vcf.gz -p $DATA/clinvar-benign-exac-filtered.vcf $DATA/MPCbenigns -i 2>/dev/null
            #gnomad benign match for ExAC v1 filter
            bedtools intersect -a $DATA/clinvar-benign-exac.vcf -b exac-ccrs.bed.gz -header -wa -sorted | bedtools intersect -a stdin -b pli.bed -header -wa -sorted | bedtools intersect -a stdin -b rvis.bed -header -wa -sorted | bedtools intersect -a stdin -b $DATA/caddbenigns -header -wa -wb -sorted > tmp/matchtmp
            cat <(grep "^#" tmp/matchtmp) <(grep -v "^#" tmp/matchtmp | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t') | bedtools intersect -a stdin -b $DATA/MPCbenigns -header -wa -wb -sorted > tmp/matchtmp2
            cat <(grep "^#" tmp/matchtmp2) <(grep -v "^#" tmp/matchtmp2 | awk '{if ($4 == $12 && $5 == $13) print $1,$2,$3,$4,$5,$6,$7,$8}' OFS='\t' | uniq) > $DATA/clinvar-benign-exac-match.vcf

            cat <(grep '^#' $DATA/clinvar-benign-exac-match.vcf) <(grep -v '^#' $DATA/clinvar-benign-exac-match.vcf | sort -k1,1 -k2,2n) | bgzip -c > $DATA/clinvar-benign-exac.vcf.gz; tabix $DATA/clinvar-benign-exac.vcf.gz
            EB=$(zgrep -v "^#" $DATA/clinvar-benign-exac.vcf.gz | wc -l)
